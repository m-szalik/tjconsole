...
public class OperationCommaand extends AbstractCommand implements Completor {
	...
	public void action(String input) throws Exception {
		...
		for(Iterator<MBeanOperationInfo> it = list.iterator(); it.hasNext();) {
			if (! it.next().getName().startsWith(methodName)) it.remove();
		}
		if (list.size() != 1) throw new IllegalArgumentException("Operations " + list + " for methodName=" + methodName);
	...
	private Object getParameter(String input, int index, MBeanParameterInfo beanParameterInfo) throws ClassNotFoundException {
		...
		Object value = ConvertUtils.convert(params.get(index), clazz);
		return value;
	}
	...
	public CommandHelp getHelp() {
		return new CommandHelp("Invoke operation.", "\\o", PREFIX) {
	...
	public final boolean matches(String input) {
		...
		try {
			for(MBeanOperationInfo oi : operations()) {
				if (oi.getName().startsWith(s)) return true;
			}
	...
	public int complete(String buffer, int cursor, List candidates) {
		...
		ArrayList<String> mycanditates = new ArrayList<String>();
		try {
			for(MBeanOperationInfo oi : operations()) {
				...
				mc.append(")"); 
				mycanditates.add(mc.toString());
			}
		} catch (Exception e) {		e.printStackTrace();	}
		
		candidates.addAll(mycanditates);
